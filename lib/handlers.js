...
//================================================================================
/*
const elasticsearchErrors = require("elasticsearch").errors;
const errorHandler = function(next) {
	return function(error) {
		console.log(error);
		if(error instanceOf elasticsearchErrors.NoConnections || error instanceOf elasticsearchErrors.NoLivingConnectionsError) {
			return next(new errors.ServiceUnavailableError("Can't connect to the Elasticsearch cluster") );
		} 
		return next( new errors.InternalServerError("Retry later or contact support") );
	};
